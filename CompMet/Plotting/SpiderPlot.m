function SpiderPlot(Metric,Names,Color)
...
TextLocRho=ones(1,length(Metric))*1.2;
TextLocTheta=deg2rad(ones(1,length(Metric))*-3);
...
text(Radius(2)/10,Radius(i)+Radius(2)/3.5,num2str(Radius(i)),  'FontSize',8,'FontName','Times')
end
...
for j=1:NSets
    ...
    x_axes(end+1)=x_axes(1);y_axes(end+1)=y_axes(1);
        ...
        for i=1:NMetrics
            if i==1
        legendhandle(j) = plot([x_axes(i) x_axes(i+1)], [y_axes(i) y_axes(i+1)],'Color', Color{j});
        else
        plot([x_axes(i) x_axes(i+1)], [y_axes(i) y_axes(i+1)],'Color', Color{j});        
        end
        end
    clear x_axes y_axes
end
